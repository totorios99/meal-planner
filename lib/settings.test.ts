// Self-check for the settings trust boundary: `node lib/settings.test.ts`.
...
// --- settingsPatch rejects what would break the planner ---
...
// --- and accepts a single-key patch, which is all the autosaving UI ever sends ---
...
// --- coerceRow falls back per field, so one bad column can't take out the page ---
assert.deepEqual(
...
// startOfWeek lands on the right day for both preferences, including the Sunday edge case.
